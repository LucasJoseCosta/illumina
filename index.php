...
$blog_page = get_page_by_path($blog_path);
$blog_id = !is_wp_error($blog_page) && $blog_page ? $blog_page->ID : null;
...
$filter_title = match ($current_lang) {
  ...
  "en" => "Filter by desired categories",
  "es" => "Filtra por las categorías deseadas",
  default => "",
};

$filter_all_post = match ($current_lang) {
  "pt" => "Todos os posts",
  "en" => "All posts",
  "es" => "Todos los posts",
  default => "",
...
$toggle_filter = match ($current_lang) {
  "pt" => "Categorias",
  "en" => "Categories",
  "es" => "Categorías",
  default => "",
...
$clean_filter = match ($current_lang) {
  "pt" => "Limpar Filtros",
  "en" => "Clear Filters",
  "es" => "Limpiar Filtros",
  default => "",
...
$apply_filter = match ($current_lang) {
  "pt" => "Aplicar Filtros",
  "en" => "Apply Filters",
  "es" => "Aplicar Filtros",
  default => "",
...
$order_recent = match ($current_lang) {
  "pt" => "Recentes",
  "en" => "Recent",
  "es" => "Recientes",
  default => "",
...
$not_found = match ($current_lang) {
  "pt" => "Antigos",
  "en" => "Older",
  "es" => "No se encontraron posts.",
  default => "",
};

$load_more = match ($current_lang) {
  "pt" => "Carregar mais",
  "en" => "Load more",
  "es" => "Cargar más",
  default => "",
...
<div class="pag-blog">
    ...
    <div class="wrapper-flex">
      <div class="wrapper-filter">
        <div class="filter-categories">
          <p class="title"><?php echo esc_html($filter_title); ?></p>
          <ul class="lista-categoria">
            <li>
              <a href="#" class="clear-categories <?php echo empty($selected_categories) ? 'selected' : ''; ?>"
                data-category="">
                <?php echo esc_html($filter_all_post); ?>
              </a>
            </li>
        ...
        <div class="filter-sorting">
          <div class="wrapper-flex-sort">
            <div class="drawer-filter">
              <button class="toggle-filter"><?php echo esc_html($toggle_filter); ?></button>
              <div class="wrapper-filter-mb">
                <a id="close-button">
                  <img class="arrow light" src="<?php echo get_template_directory_uri() ?>/assets/img/close.svg">
                  <img class="arrow dark" src="<?php echo get_template_directory_uri() ?>/assets/img/close-white.svg">
                </a>
                ...
                <div class="filter-categories">
                  ...
                  <div class="wrapper-btn">
                    <button class="clear-categories"><?php echo esc_html($clean_filter); ?></button>
                    <button class="apply-filters"><?php echo esc_html($apply_filter); ?></button>
                  </div>
            ...
            <div class="wrapper-sort">
              <select id="sort-order" name="sort_order">
                <option value="desc"
                  ...
                  <?php echo esc_html($order_recent); ?></option>
                <option value="asc"
                  <?php echo isset($_GET['sort_order']) && $_GET['sort_order'] == 'asc' ? 'selected' : ''; ?>>
                  <?php echo esc_html($order_old); ?></option>
              </select>
            </div>
          </div>
          ...
          <div class="post-grid" id="posts-container">
            <?php if ($query->have_posts()) : while ($query->have_posts()) : $query->the_post(); ?>
              ...
              <p><?php _e($not_found, 'text-domain'); ?></p>
            <?php endif; ?>
          </div>

          <?php if ($query->max_num_pages > 1) : ?>
            <div class="load-more-container">
              <button id="load-more" class="load-more-button" data-current-page="1"><?php echo esc_html($load_more); ?></button>
            </div>
          <?php endif; ?>
        </div>
      </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      const textLoadMore = currentLang === "pt" ? 'Carregando...' : currentLang === "en" ? 'Loading' : currentLang === "es" ? 'Cargando' : '';
      const textBtnLoadMore = currentLang === "pt" ? 'Carregar mais' : currentLang === "en" ? 'Load more' : currentLang === "es" ? 'Cargar más' : '';
      const textBtnLoadMoreError = currentLang === "pt" ? 'Erro ao carregar. Tente novamente.' : currentLang === "en" ? 'Error loading. Please try again.' : currentLang === "es" ? 'Error al cargar. Intenta de nuevo.' : '';
      ...
      if (loadMoreButton) {
        loadMoreButton.addEventListener('click', function() {
          ...
          loadMoreButton.textContent = textLoadMore;
          ...
          fetch(`<?php echo admin_url('admin-ajax.php'); ?>?action=load_more_posts&page=${nextPage}`, {
            ...
            .then(data => {
              ...
              loadMoreButton.textContent = textBtnLoadMore;
